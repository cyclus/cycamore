...
void SinkFacility::InitFrom(cyclus::QueryEngine* qe) {
  ...
  double cap = cyclus::GetOptionalQuery<double>(input, "input_capacity",
                                                numeric_limits<double>::max());
  capacity(cap);

  double size = cyclus::GetOptionalQuery<double>(input, "inventorysize",
                                                 numeric_limits<double>::max());
