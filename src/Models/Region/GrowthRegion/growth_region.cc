...
{
  cyclus::supply_demand::CommodityProducerManager* cast = dynamic_cast<cyclus::supply_demand::CommodityProducerManager*>(child);
  if (cast)
    {
...
{
  cyclus::action_building::Builder* cast = dynamic_cast<cyclus::action_building::Builder*>(child);
  if (cast)
    {
...
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void GrowthRegion::orderBuilds(cyclus::Commodity& commodity, double unmetdemand)
{
  vector<cyclus::action_building::BuildOrder> orders = buildmanager_.MakeBuildDecision(commodity,unmetdemand);
