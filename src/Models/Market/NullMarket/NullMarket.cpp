...
bool NullMarket::match_request(sortedMsgList::iterator request)
{
  ...
  << " which is a match from "
      ...
      Resource::Ptr res = offerMsg->trans().resource()->extractRes(requestAmt);
      ...
      LOG(LEV_DEBUG2, "NulMkt") << "NullMarket has resolved a match from "
...
void NullMarket::resolve()
{
  ...
  while (requests_.size() > 0)
  {
    ...
    if(match_request(request)) {
      ...
      LOG(LEV_DEBUG2, "NulMkt") << "The request from Requester "<< (*request).second->trans().requester()->ID()
