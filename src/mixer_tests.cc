...
namespace cycamore {
  ...
  public:
    ...
    std::vector<double> GetStream_ratio() {
      return mf_facility_->mixing_ratios;
    }
  ...
  TEST_F(MixerTest, StreamRatio) {
    ...
    EXPECT_DOUBLE_EQ(sum, 1) <<
      "Ratios normalized incorrectly: want 1, got " << sum;
  ...
  TEST_F(MixerTest, Throughput) {
    ...
    for (int i = 0; i < in_com.size(); i++) {
      ...
      EXPECT_EQ( buf_size - 0.5*buf_ratio, buf_inv) << " one (or more) input "
        "buffer inventory was not drawn from in the correct ratio.";
    }
    
    
    // output buffer size should be equal to the throuput size
    EXPECT_EQ(throughput, GetOutPutBuffer()->quantity()) << " mixing was not "
